...
async function generateTwitterPostImage({ profilePicUrl, username, handle, tweetText, tweetImageUrl, verificationType = null, affiliatedIconUrl = null }) {
  const width = 2400;
  const padding = 80;
  ...
  const headerGap = 100;
  const tweetTextY = padding + avatarSize + headerGap;
  ...
  if (affiliatedIconUrl) {
    try {
      ...
      const usernameWidth = nameMetrics.width;
      let baseX = textX + usernameWidth + 14;
      if (badgeUrl) {
        baseX = badgeX + computedBadgeSize + 14;
      }
  ...
  const tweetTextX = padding;
  drawStyledLines(ctx, lines, tweetTextX, tweetTextY, lineHeight);
...
module.exports = { generateTwitterPostImage };
